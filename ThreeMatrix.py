...
c3 = np.matrix([[45,29,89,67],[12,11,50,21],[39,17,94,55],[23,42,60,33]])

# c1 = np.random.randint(1, 100, size=(5, 5))

# # Generate the second matrix (c2)
# c2 = np.random.randint(1, 100, size=(5, 5))
...
def Table3_1(c1,c2,c3,toa1,toa2,toa3):
    df = pd.DataFrame(columns=['path','first_row','second_row','third_row','toa_first','toa_second','toa_third','eta_first','eta_second','eta_third','total_eta'])

    for i,j,n in zip(c1[0],c2[0],c3[0]):
        iteration = len(np.array(i).flatten())
        for k in range(iteration):
            path = f"0 -> {k}"
            first_metrix_row = np.array(i).flatten()[k]
            second_metrix_row = np.array(j).flatten()[k]
            third_metrix_row = np.array(n).flatten()[k]
            eta_first = 1/np.array(i).flatten()[k]
            eta_second = 1/np.array(j).flatten()[k]
            eta_third = 1/np.array(n).flatten()[k]
            total_eta = eta_first + eta_second + eta_third

            df.loc[len(df)] = {'path':path,'first_row':first_metrix_row,'second_row':second_metrix_row,'third_row':third_metrix_row,
                            'toa_first':toa1,'toa_second':toa2,'toa_third':toa3,'eta_first':eta_first,'eta_second':eta_second,'eta_third':eta_third,'total_eta':total_eta}
...
def Table3_2(df):
    ...
    for i in range(len(toa_first)):
        sum_toa_eta = sum_toa_eta + ((toa_first[i] + toa_second[i] + toa_third[i]) * (total_eta_values[i]))
    ...
    for i in range(len(toa_first)):
        prob = ((toa_first[i] + toa_second[i] + eta_third[i])*total_eta_values[i])/(sum_toa_eta)
...
while (c1.shape != (1,1)):
    ...
    x = Table3_1(c1,c2,c3,toa_first,toa_second,toa_third)
    print(x)
    ...
    deletedElementOfC3 = c3[pathNameList[0],pathNameList[1]]
    c3 = np.delete(c3,pathNameList[0],axis=0)
    c3 = np.delete(c3,pathNameList[1],axis=1)
    
    RemovedValuesOfMetrixes.append(f"({deletedElementOfC1}+{deletedElementOfC2}+{deletedElementOfC3})")
    round = round + 1
...
print("------------------------------------------------------------------------------------")
RemovedValuesOfMetrixes.append(f"({c1.item()}+{c2.item()}+{c3.item()})")
